...
class CurrencyHelper {
    ...
    public function getDefaultCurrency() {
        ...
        $stmt = $this->db->prepare("SELECT * FROM currencies WHERE id = ? AND is_active = 1 LIMIT 1");
        $stmt->execute([$default_currency_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        
        // إذا لم توجد العملة الافتراضية، استخدم الليرة اللبنانية
        if (!$result) {
            $stmt = $this->db->query("SELECT * FROM currencies WHERE currency_code = 'LBP' AND is_active = 1 LIMIT 1");
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
        }
     ...
     */
    public function createCurrencySelect($name, $selected_id = null, $class = '', $required = false, $id = null) {
        ...
        $id_attr = $id ? "id='{$id}'" : '';
        $html = "<select name='{$name}' {$id_attr} class='{$class}' {$required_attr}>";
...
function getCurrencySelect($name, $selected_id = null, $class = 'form-control', $required = false, $id = null) {
    global $currency_helper;
    return $currency_helper->createCurrencySelect($name, $selected_id, $class, $required, $id);
}
