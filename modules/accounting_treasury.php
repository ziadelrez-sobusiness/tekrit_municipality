...
// Build JS-friendly arrays for searchable dropdowns (array_values to ensure 0-indexed JSON arrays)
$income_lines_json = json_encode(array_values(array_filter($municipal_budget_lines, fn($l) => $l['section_type'] === 'income')), JSON_UNESCAPED_UNICODE);
$expense_lines_json = json_encode(array_values(array_filter($municipal_budget_lines, fn($l) => $l['section_type'] === 'expense')), JSON_UNESCAPED_UNICODE);
...
<body class="bg-slate-100 p-6">
        ...
        <div id="content-income" class="bg-white p-6 rounded shadow mb-8">
            <form method="POST" action="">
                ...
                <div class="mb-4 bg-indigo-50 border border-indigo-200 rounded p-4">
                    ...
                    <?php endif; ?>
                    <p class="text-xs text-gray-500 mb-1">اكتب جزءًا من اسم البند أو رقم الفصل أو اسم الباب للبحث بسرعة.</p>
                    <input type="text" id="income-budget-search" placeholder="ابحث في بنود الواردات حسب الباب أو الفصل أو اسم البند..." class="w-full p-2 border rounded mb-1 text-sm" oninput="filterBudgetLines('income')" autocomplete="off">
                    ...
                    <div id="income-budget-noresult" class="hidden text-red-600 text-xs mt-1">⚠️ لا توجد نتائج مطابقة لبحثك. جرب كلمة أخرى.</div>
        ...
        <div id="content-expense" class="bg-white p-6 rounded shadow mb-8 hidden">
            <form method="POST" action="">
                ...
                <div class="mb-4 bg-indigo-50 border border-indigo-200 rounded p-4">
                    ...
                    <?php endif; ?>
                    <p class="text-xs text-gray-500 mb-1">اكتب جزءًا من اسم البند أو رقم الفصل أو اسم الباب للبحث بسرعة.</p>
                    <input type="text" id="expense-budget-search" placeholder="ابحث في بنود النفقات حسب الباب أو الفصل أو اسم البند..." class="w-full p-2 border rounded mb-1 text-sm" oninput="filterBudgetLines('expense')" autocomplete="off">
                    <select name="municipal_budget_line_id" id="expense-budget-select" class="w-full p-2 border border-indigo-300 rounded text-sm">
                        <option value="">-- بدون ربط بالموازنة الرسمية --</option>
                    </select>
                    <div id="expense-budget-noresult" class="hidden text-red-600 text-xs mt-1">⚠️ لا توجد نتائج مطابقة لبحثك. جرب كلمة أخرى.</div>
                    <p class="text-xs text-indigo-600 mt-1">الموازنة البلدية الرسمية بالليرة اللبنانية. الحركات بالدولار تُحوَّل تلقائياً إلى الليرة عند احتساب الفعلي.</p>
                </div>
    ...
    <script>
        ...
        // Data is pre-split into income-only and expense-only arrays
        const incomeLinesData = <?= $income_lines_json ?>;
        const expenseLinesData = <?= $expense_lines_json ?>;

        function buildBudgetOption(line) {
            // Format: [2026] قسم الواردات | الباب 1 - chapter_name | الفصل 1.1 - item_name
            const secLabel = line.section_type === 'income' ? 'قسم الواردات' : 'قسم النفقات';
            const chapPart = line.chapter_number ? `الباب ${line.chapter_number} - ${line.chapter_name}` : line.chapter_name;
            const itemPart = line.item_number ? `الفصل ${line.item_number} - ${line.item_name}` : line.item_name;
            const label = `[${line.fiscal_year}] ${secLabel} | ${chapPart} | ${itemPart}`;
            const opt = document.createElement('option');
            opt.value = line.id;
            opt.textContent = label;
            // Build search string from all relevant fields
            opt.dataset.search = `${line.chapter_number || ''} ${line.chapter_name} ${line.item_number || ''} ${line.item_name} ${line.fiscal_year} ${secLabel}`.toLowerCase();
            return opt;
        }
        ...
        function populateBudgetSelect(type, filter) {
            ...
            const noResult = document.getElementById(type + '-budget-noresult');
            if (!sel) return 0;
            ...
            data.forEach(line => {
                ...
                if (!lowerFilter || searchable.includes(lowerFilter)) {
                    sel.appendChild(buildBudgetOption(line));
                    count++;
                }
            });
            ...
            if (noResult) {
                if (lowerFilter && count === 0) noResult.classList.remove('hidden');
        ...
        function filterBudgetLines(type) {
            const searchEl = document.getElementById(type + '-budget-search');
            if (!searchEl) return;
            populateBudgetSelect(type, searchEl.value);
        }
