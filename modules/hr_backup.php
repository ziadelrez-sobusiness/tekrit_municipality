...
try {
    ...
    if (!empty($filter_department)) {
        if ($departments_table_exists) {
            $where_conditions[] = "d.department_name = ?";
        } else {
...
<body class="bg-slate-100">
    <div class="min-h-screen p-6">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-3">
                    <button onclick="openModal('addEmployeeModal')" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition">
                        👤➕ إضافة موظف
                    </button>
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
            <h3 class="font-semibold mb-4">فلترة الموظفين</h3>
        ...
        <div class="bg-white p-6 rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">عملة الراتب</label>
                        <?= getCurrencySelect('salary_currency_id', null, 'w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500', true, 'edit_salary_currency_id') ?>
                    </div>
                </div>
    ...
    <script>
        ...
        function populateEditModal(employee) {
            ...
            document.getElementById('edit_user_type_id').value = employee.user_type_id || '';
            document.getElementById('edit_contract_type_id').value = employee.contract_type_id || '';
            document.getElementById('edit_salary').value = employee.salary || '';
            document.getElementById('edit_salary_currency_id').value = employee.salary_currency_id || '';
            document.getElementById('edit_is_active').checked = employee.is_active == 1;
            
            // تحديث عنوان Modal
            document.getElementById('editModalTitle').textContent = `تعديل الموظف: ${employee.full_name}`;
        }
