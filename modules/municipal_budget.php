...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_budget'])) {
    $appr_id = intval($_POST['budget_id']);
    try {
        $db->prepare("UPDATE municipal_budgets SET status='approved', approved_by_user_id=?, approved_at=NOW() WHERE id=?")
           ->execute([$user['id'] ?? null, $appr_id]);
        $success = 'تم اعتماد الموازنة بنجاح. أصبحت الحالة: معتمدة / فعالة.';
    } catch (Exception $e) { $error = 'حدث خطأ: ' . $e->getMessage(); }
}

// --- Return Budget to Draft ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['revert_to_draft'])) {
    $revert_id = intval($_POST['budget_id']);
    // Only safe if no linked transactions
    $chk = $db->prepare("SELECT COUNT(*) FROM financial_transactions WHERE municipal_budget_id=? OR municipal_budget_line_id IN (SELECT id FROM municipal_budget_lines WHERE municipal_budget_id=?)");
    $chk->execute([$revert_id, $revert_id]);
    if ($chk->fetchColumn() > 0) {
        $error = 'لا يمكن إرجاعها إلى مسودة لوجود حركات مالية مرتبطة.';
    } else {
        $db->prepare("UPDATE municipal_budgets SET status='draft', approved_by_user_id=NULL, approved_at=NULL WHERE id=?")->execute([$revert_id]);
        $success = 'تم إرجاع الموازنة إلى مسودة.';
    }
}
...
if ($action === 'view' && isset($_GET['id'])) {
    ...
    // Fetch actuals from financial_transactions with CORRECT currency conversion and type filtering
    // Uses budget_amount_lbp when stored, falls back to LBP amount, or on-the-fly USD conversion
    $stmt_actuals = $db->query("
        SELECT 
            ft.municipal_budget_line_id,
            mbl.section_type,
            SUM(
                COALESCE(
                    ft.budget_amount_lbp,
                    CASE 
                        WHEN c.currency_symbol IN ('LBP', 'ل.ل.', 'ل.ل') THEN ft.amount
                        WHEN c.exchange_rate_to_iqd > 0 THEN ft.amount * c.exchange_rate_to_iqd
                        ELSE 0
                    END
                )
            ) as actual_sum,
            COUNT(*) as tx_count,
            SUM(CASE WHEN c.currency_symbol NOT IN ('LBP', 'ل.ل.', 'ل.ل') AND ft.budget_amount_lbp IS NULL THEN 1 ELSE 0 END) as missing_rate_count
        ...
        WHERE ft.status NOT IN ('ملغى','cancelled') 
          AND ft.municipal_budget_line_id IS NOT NULL 
          AND (
            (mbl.section_type = 'income' AND ft.type IN ('إيراد','مدخول','income'))
            OR
            (mbl.section_type = 'expense' AND ft.type IN ('مصروف','expense','نفقة'))
          )
    ...
    while ($row = $stmt_actuals->fetch(PDO::FETCH_ASSOC)) {
        ...
        $tx_counts[$row['municipal_budget_line_id']] = intval($row['tx_count']);
    }
    ...
    foreach ($lines as $line) {
        ...
        $line['tx_count'] = $tx_counts[$line['id']] ?? 0;
...
<div class="max-w-full mx-auto px-4">
    <div class="flex justify-between items-center mb-4 bg-white p-4 rounded shadow border-l-4 border-indigo-600">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">الحالة:
                <?php
                $status_labels = ['draft'=>'مسودة', 'approved'=>'معتمدة / فعالة', 'closed'=>'مقفلة', 'cancelled'=>'ملغاة'];
                $status_colors = ['draft'=>'text-yellow-600', 'approved'=>'text-green-600', 'closed'=>'text-gray-500', 'cancelled'=>'text-red-600'];
                $sl = $budget['status'];
                ?>
                <span class="font-bold <?= $status_colors[$sl] ?? 'text-gray-700' ?>"><?= $status_labels[$sl] ?? $sl ?></span>
                <?php if ($budget['approved_at']): ?>
                    <span class="text-xs text-gray-400 mr-2">تاريخ الاعتماد: <?= $budget['approved_at'] ?></span>
                <?php endif; ?>
                | الموازنة البلدية الرسمية تُعدّ <span class="font-bold text-red-600">بالليرة اللبنانية</span> فقط.
            </p>
        </div>
        <div class="flex flex-wrap gap-2">
            ...
            <form method="POST">
                ...
                <button type="submit" name="approve_budget" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 font-bold" onclick="return confirm('هل تريد اعتماد هذه الموازنة وتفعيلها؟')">&#10003; اعتماد الموازنة</button>
            </form>
            <?php elseif ($sl === 'approved'): ?>
            <span class="bg-green-100 text-green-800 px-4 py-2 rounded font-bold text-sm">✅ معتمدة / فعالة</span>
            <?php if ($is_admin): ?>
            <form method="POST">
                <input type="hidden" name="budget_id" value="<?= $budget_id ?>">
                <button type="submit" name="revert_to_draft" class="bg-yellow-100 text-yellow-700 px-3 py-2 rounded text-sm hover:bg-yellow-200" onclick="return confirm('إرجاع إلى مسودة - يسمح فقط إذا لا توجد حركات مرتبطة.')">&#8592; مسودة</button>
            </form>
            <?php endif; ?>
            <?php elseif ($sl === 'cancelled'): ?>
            <span class="bg-red-100 text-red-700 px-4 py-2 rounded font-bold text-sm">❌ ملغاة</span>
            <?php elseif ($sl === 'closed'): ?>
            <span class="bg-gray-200 text-gray-700 px-4 py-2 rounded font-bold text-sm">🔒 مقفلة</span>
            <?php endif; ?>
            <a href="?action=view&id=<?= $budget_id ?>&print=1" target="_blank" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">🖨️ طباعة</a>
            <a href="municipal_budget.php" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">عودة</a>
        </div>
    </div>
                        ...
                        <table class="w-full text-right text-sm border-collapse">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="border p-2 w-40">الفعلي (ل.ل.) / الحركات</th>
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-50 border-b <?= !$is_active?'inactive-row':'' ?>">
                                    ...
                                    <td class="p-2 text-left font-bold" dir="ltr">
                                        ...
                                        <?php if($line['tx_count'] > 0): ?>
                                            <a href="accounting_cashbox_statement.php?budget_line_id=<?= $line['id'] ?>&quick=all" target="_blank" class="mr-1 inline-block bg-blue-100 text-blue-700 text-xs px-1 py-0.5 rounded" title="الحركات المرتبطة"><?= $line['tx_count'] ?> حركة</a>
                                        <?php endif; ?>
