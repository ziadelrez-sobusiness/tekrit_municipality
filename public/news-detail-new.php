...
<body class="bg-gray-50">
                        ...
                        <article class="bg-white rounded-lg shadow-md overflow-hidden card-hover">
                            ...
                            <div class="p-4">
                                ...
                                <h3 class="text-lg font-semibold text-gray-900 mb-2 hover:text-indigo-600">
                                    <a href="news-detail-new.php?id=<?= $item['id'] ?>" class="line-clamp-2">
                                        <?= htmlspecialchars($item['title']) ?>
                                    </a>
                                </h3>
                                <p class="text-gray-600 text-sm mb-3 line-clamp-2">
                                    <?= htmlspecialchars(substr($item['content'], 0, 120)) ?>...
                                </p>
                                <div class="flex justify-between items-center text-xs text-gray-500">
                                    <span><?= date('Y/m/d', strtotime($item['publish_date'])) ?></span>
                                    <span><?= number_format($item['views_count']) ?> مشاهدة</span>
                                </div>
                            </div>
                        </article>
                    <?php endforeach; ?>
    ...
    <!-- Image Modal -->
    <div id="imageModal" class="modal fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center">
        <div class="modal-content relative">
            ...
            <div id="modalCaption" class="absolute bottom-4 left-4 right-4 text-white text-center bg-black bg-opacity-60 p-2 rounded"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function shareOnFacebook() {
            ...
            const title = encodeURIComponent('<?= htmlspecialchars($news['title']) ?>');
            window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${title}`, '_blank', 'width=600,height=400');
        }

        function shareOnTwitter() {
            const url = encodeURIComponent(window.location.href);
            const text = encodeURIComponent('<?= htmlspecialchars($news['title']) ?>');
