        ...
        if ($amount > 0 && $currency_id && $cashbox_id && $category_id && $payer_name) {
            ...
            if ($actual_cb_currency != $currency_id) {
                ...
                try {
                    ...
                    // Get exchange rate if linked to municipal budget
                    $budget_exchange_rate = null;
                    $budget_amount_lbp = null;
                    if ($municipal_budget_line_id) {
                        $check_curr = $db->prepare("SELECT currency_symbol, exchange_rate FROM currencies WHERE id = ?");
                        $check_curr->execute([$currency_id]);
                        $curr_info = $check_curr->fetch(PDO::FETCH_ASSOC);
                        if ($curr_info && ($curr_info['currency_symbol'] === 'LBP' || $curr_info['currency_symbol'] === 'ل.ل.')) {
                            $budget_exchange_rate = 1;
                            $budget_amount_lbp = $amount;
                        } else {
                            $budget_exchange_rate = floatval($curr_info['exchange_rate'] ?? 89500);
                    ...
                    $stmt = $db->prepare("INSERT INTO financial_transactions 
                        (type, category, amount, currency_id, cashbox_id, committee_id, project_id, budget_item_id, committee_budget_item_id, municipal_budget_line_id, budget_exchange_rate, budget_amount_lbp, transaction_date, payment_method, description, created_by, status)
                        VALUES ('إيراد', (SELECT name_ar FROM accounting_categories WHERE id=?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'معتمد')");
                    $stmt->execute([$category_id, $amount, $currency_id, $cashbox_id, $committee_id, $project_id, $budget_item_id, $committee_budget_item_id, $municipal_budget_line_id, $budget_exchange_rate, $budget_amount_lbp, $transaction_date, $payment_method, $notes, $user['id']]);
...
try {
    $municipal_budget_lines = $db->query("
        SELECT l.id, l.section_type, l.chapter_name, l.item_number, l.item_name, b.fiscal_year
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div id="content-expense" class="bg-white p-6 rounded shadow mb-8 hidden">
            <form method="POST" action="">
                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-4 mb-4 bg-indigo-50 p-4 rounded border border-indigo-200">
                    <div>
                        <label class="block text-sm font-medium mb-1 text-indigo-800">بند الموازنة البلدية الرسمية - مصروف</label>
                        <select name="municipal_budget_line_id" class="w-full p-2 border border-indigo-300 rounded">
                            <option value="">-- اختياري: يفضل الاختيار لاحتساب الحركة ضمن موازنة البلدية --</option>
                            <?php foreach($municipal_budget_lines as $line): if($line['section_type'] === 'expense'): ?>
                                <option value="<?= $line['id'] ?>"><?= $line['fiscal_year'] ?> - <?= htmlspecialchars($line['chapter_name']) ?> - <?= htmlspecialchars($line['item_name']) ?></option>
