...
function formatProjectCost($project, $db = null) {
    ...
    $amount = $project['budget'] ?? ($project['project_cost'] ?? 0);
    $currency_id = $project['budget_currency_id'] ?? ($project['currency_id'] ?? null);
...
function calculateTotalBudget($db = null) {
    ...
    $projects = $db->query("SELECT budget, budget_currency_id FROM projects WHERE budget > 0")->fetchAll();
    
    $total_in_lbp = 0;
    
    foreach ($projects as $project) {
        $cost = $project['budget'] ?? $project['project_cost'];
        $currency_id = $project['budget_currency_id'] ?? ($project['currency_id'] ?? $display_currency_id);
...
function updateProjectCurrency($project_id, $new_currency_id, $db = null) {
    ...
    $stmt = $db->prepare("UPDATE projects SET budget_currency_id = ? WHERE id = ?");
