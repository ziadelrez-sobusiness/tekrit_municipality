...
// Expected to be included from municipal_budget.php
if (!isset($budget)) die();
?>
<!DOCTYPE html>
...
<style>
    ...
    .summary-box { border: 1px solid #000; padding: 10px; width: 300px; margin: 20px auto; text-align: center; font-weight: bold; }
...
<div class="meta">
    ...
    <div>حالة الموازنة: <?= $budget['status'] ?></div>
</div>

<div class="summary-box">
    <div style="font-size:14px; margin-bottom:10px;">الخلاصة</div>
    <div>مجموع الواردات المقدرة: <span dir="ltr"><?= number_format($totals['income_est'], 2) ?></span></div>
    <div>مجموع النفقات المقدرة: <span dir="ltr"><?= number_format($totals['expense_est'], 2) ?></span></div>
    <div style="border-top:1px solid #000; margin-top:5px; padding-top:5px;">الفارق: <span dir="ltr"><?= number_format($totals['income_est'] - $totals['expense_est'], 2) ?></span></div>
</div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th width="100">تقديرات الموازنة السابقة</th>
                    <th width="100">تقديرات موازنة السنة الجارية</th>
                    <th>شرح البند</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($grouped[$sec_key] as $chap_name => $chap_data): ?>
                    ...
                    <tr>
                        ...
                        <td dir="ltr"><?= number_format($line['previous_estimate'], 2) ?></td>
                        <td dir="ltr"><?= number_format($line['current_estimate'], 2) ?></td>
                        <td><?= htmlspecialchars($line['explanation'] ?? '') ?></td>
                    </tr>
                    <?php endforeach; ?>
                    <tr class="total-row">
                        <td colspan="3">مجموع <?= htmlspecialchars($chap_name) ?></td>
                        <td dir="ltr">0.00</td> <!-- Assuming prev estimate total not tracked in sum array -->
                        <td dir="ltr"><?= number_format($chap_data['est_sum'], 2) ?></td>
                        <td></td>
                    </tr>
                <?php endforeach; ?>
                <tr class="total-row" style="background-color:#d0d0d0;">
                    ...
                    <td dir="ltr">-</td>
                    <td dir="ltr"><?= number_format($totals[$sec_key.'_est'], 2) ?></td>
                    <td></td>
                </tr>
            </tbody>
        </table>
