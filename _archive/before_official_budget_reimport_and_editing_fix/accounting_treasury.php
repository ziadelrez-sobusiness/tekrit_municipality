            ...
            if ($actual_cb_currency != $currency_id) {
                ...
                try {
                    ...
                    $stmt = $db->prepare("INSERT INTO financial_transactions 
                        (type, category, amount, currency_id, cashbox_id, committee_id, project_id, budget_item_id, committee_budget_item_id, municipal_budget_line_id, transaction_date, payment_method, description, created_by, status)
                        VALUES ('إيراد', (SELECT name_ar FROM accounting_categories WHERE id=?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'معتمد')");
                    $stmt->execute([$category_id, $amount, $currency_id, $cashbox_id, $committee_id, $project_id, $budget_item_id, $committee_budget_item_id, $municipal_budget_line_id, $transaction_date, $payment_method, $notes, $user['id']]);
