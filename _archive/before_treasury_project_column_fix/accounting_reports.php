...
$stmt = $db->prepare("
    SELECT ft.*, c.currency_symbol, mc.committee_name, p.title as project_name, cb.name as cashbox_name, r.receipt_number, v.voucher_number
    FROM financial_transactions ft
...
$projects = [];
try { $projects = $db->query("SELECT id, title as project_name FROM projects")->fetchAll(PDO::FETCH_ASSOC); } catch(Exception $e){}
