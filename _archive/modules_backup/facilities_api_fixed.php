...
try {
    ...
    switch ($action) {
        case 'get_facilities':
            ...
            $lang = $_GET['lang'] ?? 'ar';
            $limit = (int) min(500, max(1, $_GET['limit'] ?? 100)); // تحويل إلى integer
            ...
            $response['total'] = count($formatted_facilities);
            break;
        ...
        case 'search_nearby':
            ...
            $radius = (float) min(50, max(1, $_GET['radius'] ?? 5)); // نطاق من 1 إلى 50 كم
            $category_id = $_GET['category'] ?? '';
