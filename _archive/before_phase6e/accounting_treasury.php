        ...
        if ($amount > 0 && $currency_id && $cashbox_id && $category_id && $payer_name) {
            try {
                ...
                $stmt = $db->prepare("INSERT INTO financial_transactions 
                    (type, category, amount, currency_id, cashbox_id, committee_id, project_id, budget_item_id, transaction_date, payment_method, description, created_by, status)
                    VALUES ('إيراد', (SELECT name_ar FROM accounting_categories WHERE id=?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'معتمد')");
                $stmt->execute([$category_id, $amount, $currency_id, $cashbox_id, $committee_id, $project_id, $budget_item_id, $transaction_date, $payment_method, $notes, $user['id']]);
            ...
            <form method="POST" action="">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="currency_id" required class="w-full p-2 border rounded" onchange="filterCashboxes(this.value, 'income-cashbox')">
        ...
        <div id="content-expense" class="bg-white p-6 rounded shadow mb-8 hidden">
            <form method="POST" action="">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1">العملة <span class="text-red-500">*</span></label>
                        <select name="currency_id" required class="w-full p-2 border rounded" onchange="filterCashboxes(this.value, 'expense-cashbox')">
                            <option value="">اختر العملة...</option>
                            <?php foreach ($currencies as $curr): ?>
                                <option value="<?= $curr['id'] ?>"><?= $curr['currency_name'] ?> (<?= $curr['currency_symbol'] ?>)</option>
                            <?php endforeach; ?>
