...
// Accept transaction_id or id for backward compatibility
$id = intval($_GET['transaction_id'] ?? $_GET['id'] ?? 0);
$from_cashbox = intval($_GET['from_cashbox'] ?? 0);
$error = ''; $message = '';
...
// Robust: treat both Arabic and English income/expense labels as income
$is_cancelled = in_array($tx['status'] ?? '', ['ملغى', 'cancelled']);
$is_income    = in_array($tx['type'] ?? '', ['إيراد', 'مدخول', 'income']);
$new_balance  = null;

// Generate a page-scoped cancel token stored in session
if (!isset($_SESSION['cancel_nonce_' . $id])) {
    $_SESSION['cancel_nonce_' . $id] = bin2hex(random_bytes(16));
}
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded shadow p-5">
        ...
        <form method="POST" action="accounting_transaction_cancel.php?transaction_id=<?= $id ?>&from_cashbox=<?= $from_cashbox ?>">
            <input type="hidden" name="cancel_nonce" value="<?= htmlspecialchars($_SESSION['cancel_nonce_' . $id] ?? '') ?>">
            ...
            <div class="mb-4">
                ...
                <textarea name="cancellation_reason" required rows="3"
                    placeholder="مثال: خطأ في المبلغ، خطأ في الصندوق، تكرار..."
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <a href="<?= htmlspecialchars($back_url) ?>" class="bg-gray-200 text-gray-700 px-5 py-2 rounded hover:bg-gray-300">رجوع</a>
            </div>
        </form>
    </div>
    <?php endif; ?>
