...
@media print {
    ...
    body { background: white; padding: 8px; font-size: 11px; }
}
</style>
</head>
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-4 no-print">
        <div>
            ...
            <?php if ($sel_cashbox): ?>
            <p class="text-sm text-indigo-600 font-semibold"><?= htmlspecialchars($sel_cashbox['name']) ?> (<?= $sel_cashbox['currency_symbol'] ?>)</p>
            <?php endif; ?>
        </div>
        <div class="flex gap-2">
            <button onclick="window.print()" class="bg-indigo-600 text-white px-3 py-2 rounded hover:bg-indigo-700 text-sm">🖨️ طباعة</button>
            <a href="accounting_cashboxes.php" class="bg-gray-600 text-white px-3 py-2 rounded hover:bg-gray-700 text-sm">إدارة الصناديق</a>
            <a href="../comprehensive_dashboard.php" class="bg-gray-500 text-white px-3 py-2 rounded hover:bg-gray-600 text-sm">الرئيسية</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
        <div class="bg-indigo-50 border border-indigo-200 p-3 rounded shadow text-center">
            <div class="text-xs text-indigo-500 mb-1">الرصيد الحالي</div>
            <div class="font-bold text-indigo-700 text-sm" dir="ltr"><?= number_format($current_balance, 2) ?> <?= $sel_cashbox['currency_symbol'] ?></div>
        </div>
        <div class="bg-green-50 border border-green-200 p-3 rounded shadow text-center">
            <div class="text-xs text-green-600 mb-1">مجموع المداخيل</div>
            <div class="font-bold text-green-700 text-sm" dir="ltr">+ <?= number_format($total_income, 2) ?></div>
        </div>
        <div class="bg-red-50 border border-red-200 p-3 rounded shadow text-center">
            <div class="text-xs text-red-600 mb-1">مجموع المصاريف</div>
            <div class="font-bold text-red-700 text-sm" dir="ltr">- <?= number_format($total_expense, 2) ?></div>
        </div>
        <?php $net = $total_income - $total_expense; ?>
        <div class="bg-blue-50 border border-blue-200 p-3 rounded shadow text-center">
            <div class="text-xs text-blue-600 mb-1">صافي الحركة</div>
            <div class="font-bold text-sm <?= $net>=0?'text-green-700':'text-red-700' ?>" dir="ltr"><?= ($net>=0?'+':'') . number_format($net,2) ?></div>
        </div>
        <div class="bg-gray-50 border border-gray-200 p-3 rounded shadow text-center">
            <div class="text-xs text-gray-500 mb-1">عدد الحركات</div>
            <div class="font-bold text-gray-700"><?= $count_active ?></div>
        </div>
        <div class="bg-orange-50 border border-orange-200 p-3 rounded shadow text-center">
            <div class="text-xs text-orange-500 mb-1">حركات ملغاة</div>
            <div class="font-bold text-orange-700"><?= $count_cancelled ?></div>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50 <?= $is_cancelled ? 'cancelled-row' : '' ?>">
                    ...
                    <td class="p-2 no-print">
                        <div class="flex gap-1 flex-wrap no-strike">
                            <a href="accounting_transaction_view.php?transaction_id=<?= $tx['id'] ?>" class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded hover:bg-blue-200">عرض</a>
                            <?php if (!$is_cancelled): ?>
                                <?php if ($is_income && $doc_num): ?>
                                    <a href="print_receipt.php?transaction_id=<?= $tx['id'] ?>" target="_blank" class="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded hover:bg-indigo-200">🖨️ إيصال</a>
                                <?php elseif (!$is_income && $doc_num): ?>
                                    <a href="print_voucher.php?transaction_id=<?= $tx['id'] ?>" target="_blank" class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded hover:bg-purple-200">🖨️ سند</a>
                                <?php endif; ?>
                                <a href="accounting_transaction_view.php?transaction_id=<?= $tx['id'] ?>&edit=1" class="bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded hover:bg-yellow-200">تعديل</a>
                                <a href="accounting_transaction_cancel.php?transaction_id=<?= $tx['id'] ?>&from_cashbox=<?= $sel_cashbox_id ?>" class="bg-red-100 text-red-700 px-2 py-0.5 rounded hover:bg-red-200 font-bold"
                                   onclick="return confirm('هل تريد إلغاء هذه الحركة المالية؟\nسيتم عكس رصيد الصندوق تلقائياً.')">&#x26D4; إلغاء</a>
                            <?php else: ?>
