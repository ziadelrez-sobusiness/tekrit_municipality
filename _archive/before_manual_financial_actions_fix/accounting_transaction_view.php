...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-5">
        ...
        <div class="flex gap-2 flex-wrap">
            <?php if (!$is_cancelled && !$edit_mode): ?>
                <a href="?id=<?= $id ?>&edit=1" class="bg-yellow-500 text-white px-3 py-2 rounded hover:bg-yellow-600 text-sm">✎ تعديل</a>
                <a href="accounting_transaction_cancel.php?id=<?= $id ?>&from_cashbox=<?= $tx['cashbox_id'] ?>"
    ...
    <div class="bg-white rounded shadow p-6 mb-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            <div><span class="text-xs text-gray-500 block"><?= $is_income?'رقم الإيصال':'رقم سند الصرف' ?></span>
    ...
    <div class="mb-4 flex gap-2">
        <?php if ($is_income && $doc_num): ?>
            <a href="print_receipt.php?transaction_id=<?= $id ?>" target="_blank" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 text-sm">🖨️ طباعة الإيصال</a>
        <?php elseif (!$is_income && $doc_num): ?>
            <a href="print_voucher.php?transaction_id=<?= $id ?>" target="_blank" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 text-sm">🖨️ طباعة سند الصرف</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="bg-white rounded shadow p-6 mb-4 border-t-4 border-yellow-400">
        ...
        <form method="POST">
            ...
            <div class="mt-4 flex gap-2">
                ...
                <a href="?id=<?=$id?>" class="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300">إلغاء</a>
            </div>
        </form>
    </div>
    <?php endif; ?>
